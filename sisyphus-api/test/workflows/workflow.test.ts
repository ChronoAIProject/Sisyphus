...
vi.mock("../../src/workflows/ornn-client.js", () => ({
...
describe("POST /workflows/compile/:workflowId", () => {
  let workflowId: string;
  ...
  it("compiles a workflow with skill injection", async () => {
    ...
    const res = await request(app).get(`/workflows/compile/${workflowId}`);
  ...
  it("returns 404 for unknown workflow", async () => {
    const app = await getApp();
    const res = await request(app).get("/workflows/compile/nonexistent");
    expect(res.status).toBe(404);
  });
});
