...
import { isTectonicAvailable } from "../../src/papers/compiler.js";
...
describe("POST /papers/compile", () => {
  it("returns 400 for missing nodes", async () => {
    const res = await request(app).post("/papers/compile").send({ edges: [] });
    expect(res.status).toBe(400);
    expect(res.body.error).toBeDefined();
  });

  it("returns 400 for empty nodes array", async () => {
    const res = await request(app).post("/papers/compile").send({ nodes: [], edges: [] });
    expect(res.status).toBe(400);
  });
