...
vi.mock("../../src/ingest/chrono-graph-client.js", () => ({
...
describe("GET /ingest/history", () => {
  it("returns paginated upload history", async () => {
    const app = await getApp();
    const res = await request(app).get("/ingest/history");
...
describe("GET /ingest/history/:uploadId", () => {
  it("returns details for a specific upload", async () => {
    ...
    const res = await request(app).get(`/ingest/history/${uploadId}`);
    expect(res.status).toBe(200);
  ...
  it("returns 404 for unknown upload id", async () => {
    const app = await getApp();
    const res = await request(app).get("/ingest/history/nonexistent");
